...
i=0
until curl -fsS "http://127.0.0.1:${BACKEND_PORT:-19507}/healthz" >/dev/null; do
  i=$((i+1)); [ "$i" -lt 60 ] || { docker compose logs; exit 1; }; sleep 2
done
curl -fsS "http://127.0.0.1:${FRONTEND_PORT:-18507}/" >/dev/null
token=$(curl -fsS -X POST "http://127.0.0.1:${BACKEND_PORT:-19507}/api/auth/login" -H 'Content-Type: application/json' -d '{"username":"admin","password":"Admin123!"}' | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')
[ -n "$token" ]
curl -fsS "http://127.0.0.1:${BACKEND_PORT:-19507}/api/overview" -H "Authorization: Bearer $token" >/dev/null
