...
func Load() (Config, error) {
	cfg := Config{
		AppName:           env("APP_NAME", "foundry-melt-quality-control"),
		...
		LoginRequestLimit: intEnv("LOGIN_REQUEST_LIMIT", 10),
...
func (c Config) Validate() error {
	...
	if c.IsProduction() && (len(c.JWTSecret) < 32 || strings.Contains(strings.ToLower(c.JWTSecret), "development") ||
		strings.Contains(strings.ToLower(c.JWTSecret), "replace-this")) {
	...
	if c.LoginRequestLimit < 3 || c.LoginRequestLimit > 1000 {
		return fmt.Errorf("LOGIN_REQUEST_LIMIT must be between 3 and 1000")
	}
