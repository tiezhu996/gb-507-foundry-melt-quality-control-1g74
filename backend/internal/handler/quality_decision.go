...
type QualityDecisionHandler struct {
	service service.QualityDecisionService
}

func NewQualityDecisionHandler(s service.QualityDecisionService) *QualityDecisionHandler {
	return &QualityDecisionHandler{service: s}
}

func (h *QualityDecisionHandler) Register(group *gin.RouterGroup) {
...
func (h *QualityDecisionHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *QualityDecisionHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *QualityDecisionHandler) create(c *gin.Context) {
	var input dto.CreateQualityDecision
...
func (h *QualityDecisionHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateQualityDecision
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *QualityDecisionHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
