...
type SystemHandler struct {
	...
	heat            service.HeatService
	chemicalSample  service.ChemicalSampleService
	qualityDecision service.QualityDecisionService
	db              *gorm.DB
	redis           *redis.Client
}

func NewSystemHandler(security service.SecurityService, furnace service.FurnaceService, heat service.HeatService, chemicalSample service.ChemicalSampleService, qualityDecision service.QualityDecisionService, db *gorm.DB, redisClient *redis.Client) *SystemHandler {
	return &SystemHandler{security: security, furnace: furnace, heat: heat, chemicalSample: chemicalSample, qualityDecision: qualityDecision, db: db, redis: redisClient}
}
...
func (h *SystemHandler) Overview(c *gin.Context) {
	...
	furnaceCounts, err := h.furnace.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["furnaces"] = furnaceCounts

	heatCounts, err := h.heat.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["heats"] = heatCounts

	chemicalSampleCounts, err := h.chemicalSample.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["samples"] = chemicalSampleCounts

	qualityDecisionCounts, err := h.qualityDecision.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["decisions"] = qualityDecisionCounts
